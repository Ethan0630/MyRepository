export const fetchIndustry = async () => {
    try {
        const response = await fetch("http://localhost:9060/idt/getIndustry");
...
export const uploadIndustry = async (industryName) => {
    try {
        ...
        const response = await fetch("http://localhost:9060/admin/idt/upload", {
            method: "POST",
            headers: {
                // ✅ 關鍵：要帶上 token！
...
export const deleteIndustry = async (selectedIndustry) => {
    ...
    try {
        ...
        const response = await fetch(`http://localhost:9060/admin/idt/delete/${selectedIndustry}`, {
            method: "DELETE",
            headers: {
                // ✅ 關鍵：要帶上 token！
                "Authorization": "Bearer " + token
