export const uploadProject = async (name, selectedIndustry, selectedCategory, files, description) => {
    try {
        ...
        const response = await fetch("http://3.113.186.54:9060/admin/project/upload", {
            method: "POST",
            headers: {
                // ✅ 關鍵：要帶上 token！
...
export const updateProject = async (selectedId, selectedIndustry, selectedCategory, files, description) => {
    try {
        ...
        const response = await fetch("http://3.113.186.54:9060/admin/project/update", {
            method: "PUT",
            headers: {
                // ✅ 關鍵：要帶上 token！
                "Authorization": "Bearer " + token
...
export const fetchProjectName = async () => {
    const response = await fetch("http://3.113.186.54:9060/project/getAllName")
...
export const fetchProjectById = async (selectedId) => {
    ...
    const response = await fetch(`http://3.113.186.54:9060/project/getProjectById?id=${selectedId}`);
    if (!response.ok) {
        throw new Error("獲取失敗！");
    }
...
export const deleteProjectById = async (selectedId) => {
    ...
    try {
        ...
        const response = await fetch(`http://3.113.186.54:9060/admin/project/delete?id=${selectedId}`, {
            method: "DELETE",
            headers: {
                // ✅ 關鍵：要帶上 token！
                "Authorization": "Bearer " + token
